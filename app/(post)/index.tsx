...
export default function PostsScreen() {
  ...
  const handleSortChange = (order: "asc" | "desc") => {
    ...
    setShowSortModal(false); // Close modal if open
  };
    ...
    <View className="flex-1 bg-gray-50">
      ...
      >
        ...
        {authorFilter?.length > 0 ? (
          <TouchableOpacity onPress={() => setAuthorFilter("")} style={styles.clearButton}>
            <Ionicons name="close-circle" size={20} color="#319795" />
          </TouchableOpacity>
        ) : null}
      </View>
            ...
            <Modal visible={showSortModal} transparent={true} animationType="slide">
              <View style={styles.modalContainer}>
                <View style={styles.modalContent}>
                  ...
                  />
                  <Button title="Cancel" onPress={() => setShowSortModal(false)} />
                </View>
              </View>
            </Modal>
          </>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity className="bg-white rounded-lg shadow-md overflow-hidden mb-4" onPress={() => router.push("/(post)")}>
              <Image
